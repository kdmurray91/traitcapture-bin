...
class RectangleBuilder:
    ...
    def __init__(self, Rectangle, AspectRatio, Image):
        ...
        self.Image = Image
        self.RectList = []
        self.cid = Rectangle.figure.canvas.mpl_connect(
            'button_press_event',
            self.onMouseClicked)
        self.cid = Rectangle.figure.canvas.mpl_connect(
            'key_press_event',
...
#        print('click', event)
        ...
        if self.AspectRatio is not None and len(self.lclick_x) == 2:
...
#        cv2.imwrite('/home/chuong/Data/GC03L-temp/corrected/'+CornerType+'.jpg', window)
    ...
    def getRectCornersFrom2Points(self, lclick_x, lclick_y):
        ...
        Centre = np.asarray(
            [lclick_x[0] + lclick_x[1],
             lclick_y[0] + lclick_y[1]]) / 2.0
        Angle = np.arctan2(Height, Width) - \
            np.arctan2(
                lclick_y[1] - lclick_y[0],
                lclick_x[1] - lclick_x[0])
    ...
    def createRectangle(self, Centre, Width, Height, Angle):
        ...
        for i in range(len(RectFit)):
            ...
            xrot =  RectFit[i][0] * \
                np.cos(Angle) + RectFit[i][1] * np.sin(Angle)
            yrot = -RectFit[i][0] * \
                np.sin(Angle) + RectFit[i][1] * np.cos(Angle)
            RectFit[i][0], RectFit[i][1] = (
                xrot + Centre[0]), (yrot + Centre[1])
        return RectFit
...
def selectColorCard(Img, AspectRatio):
    ...
    ax.set_title(
        'left click at top-left corner and right click at bottom-right corner to to select pot area')
    ax.imshow(Img)
...
def main(argv):
    ...
    for i in range(24):
        ...
        Colors.append(
            np.asarray([P24ColorCard[rr, cc, 0], P24ColorCard[rr, cc, 1], P24ColorCard[rr, cc, 2]], dtype=np.float))
    print('Colors = \n', Colors)
    ...
    for Rect in RectList:
        ...
        for i in range(24):
            ...
            Captured_R = RectifiedColorCard[
                ...
                0].astype(
                np.float)
            Captured_G = RectifiedColorCard[
                rr - 10:rr + 10,
                cc - 10:cc + 10,
                1].astype(
                np.float)
            Captured_B = RectifiedColorCard[
                rr - 10:rr + 10,
                cc - 10:cc + 10,
                2].astype(
                np.float)
            ...
            Captured_Colors.append(
                np.asarray([Captured_R, Captured_G, Captured_B], dtype=np.float))
        ...
        ArgRefined, _ = optimize.leastsq(
            getColorMatchingError, Arg, args=(Colors, Captured_Colors), maxfev=10000)
