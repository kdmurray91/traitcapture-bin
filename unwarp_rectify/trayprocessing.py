...
def joinTrayImages(TrayImageList, Shape=[2, 4]):
    ...
    RectifiedImage = np.resize(
        np.zeros_like(TrayImageList[0]),
        (Shape[0] * TrayShape[0],
         Shape[1] * TrayShape[1],
         TrayShape[2]))
    for r in range(Shape[0]):
        for c in range(Shape[1]):
            ...
            cc0 = c * TrayShape[1]
            cc1 = (c + 1) * TrayShape[1]
            RectifiedImage[rr0:rr1,
                           cc0:cc1,
                           :] = TrayImageList[c + r * Shape[1]]
    ...
    def next(self):
        if self.num < self.n and os.path.exists(self.FileNameList[self.num]):
            ...
            if abs(self.RotationAngle) == 180:
                ...
                ImageUndistorted = rotateImage(
                    ImageUndistorted,
                    self.RotationAngle)

            RectifiedTrayImageList = rectifyTrayImages(
                ImageUndistorted,
                self.TrayRectangleList,
                self.TrayImgSize)
            RectifiedImage = joinTrayImages(
                RectifiedTrayImageList,
                self.TrayArrangement)
...
def demo_parallel(
        ImageFileListIn, ImageFileListOut, CameraMatrix, DistortionCoeffients,
        ImageSize, RotationAngle, TrayRectangleList, TrayImgSize, TrayArrangement):

    MapX, MapY = cv2.initUndistortRectifyMap(
        CameraMatrix, DistortionCoeffients,
...
if __name__ == "__main__":
    ...
    ImageFileListOut = [os.path.join(OutputFolder, os.path.basename(ImageFile))
                        for ImageFile in ImageFileListIn]
    ImageSize, SquareSize, CameraMatrix, DistortionCoeffients, RVecs, TVecs = readCalibFile(
        CalibFile)
    TrayRectangleList, TrayImgWidth, TrayImgHeight = readTrayConfigFile(
        ConfigFile)
...
# ImageSize, RotationAngle, TrayRectangleList, TrayImgSize,
# TrayArrangement)
    demo_generator(
        ImageFileListIn, ImageFileListOut, CameraMatrix, DistortionCoeffients,
        ImageSize, RotationAngle, TrayRectangleList, TrayImgSize, TrayArrangement)
